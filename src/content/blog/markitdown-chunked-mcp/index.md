---
title: "自己动手写一个 MCP：解决大文件解析的上下文爆炸问题"
description: "从发现 markitdown MCP 无法处理大文件，到自己写一个支持章节拆分和 AI 摘要的 MCP server 的完整过程。"
date: "2026-04-07"
tags: ["技术"]
---
...
我在用 Kiro 搭建了一套"搜书→下载→阅读"的工作流：通过 Telegram Bot 搜书，下载 epub/pdf 到本地，再用 markitdown MCP 转成 markdown 来分析内容。

流程在小文件上跑得很顺。但当我试着解析一本《毛泽东选集》（1-5卷，epub 格式）时，直接炸了——markitdown MCP 会把整本书的转换结果一次性塞进对话上下文，2MB 的文本瞬间撑爆了 LLM 的上下文窗口。

这不是 markitdown 本身的 bug，而是架构上的限制：MCP 工具的返回值会直接注入上下文，大文件天然不适合这种模式。
...
拆分完章节后，自然想让 AI 帮忙总结每章内容。但 222 个章节逐章总结，单次对话的上下文肯定不够。
...
整个流程通过 Kiro 的 steering 规则串联，说一句"帮我找《三体》"就能自动走完搜索和下载，再说"解析一下"就拆分章节，"帮我总结"就开始逐章摘要。
